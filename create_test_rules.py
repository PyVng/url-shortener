...
def create_test_rules():
    ...
    try:
        ...
        test_url = Url.create_short_url(
            db, "https://example.com/original", "http://localhost:8001", None  # No user
        )
        ...
        print(
            f"curl -H 'X-Forwarded-For: 1.2.3.4' http://localhost:8001/{test_url.short_code}"
        )
        print(
            f"curl -H 'User-Agent: Mozilla/5.0 (iPhone...' http://localhost:8001/{test_url.short_code}"
        )
        print(
            f"curl -H 'Referer: https://google.com' http://localhost:8001/{test_url.short_code}"
        )
